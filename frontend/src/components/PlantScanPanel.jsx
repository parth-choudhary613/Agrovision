        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl border border-gray-100 shadow-sm overflow-hidden mb-6 ">
          ...
          <div className="p-4 sm:p-6">
            <video
              ...
              autoPlay
              playsInline
              className="w-full h-[300px] sm:h-[400px] object-cover rounded-xl sm:rounded-2xl bg-black"
            />
            <canvas ref={canvasRef} className="hidden" />
            <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4 mt-5 sm:mt-6">
              <button
                onClick={capturePhoto}
                className="w-full sm:w-auto bg-green-700 hover:bg-green-800 text-white px-8 py-3 rounded-xl sm:rounded-2xl font-semibold flex justify-center items-center gap-2 transition-colors"
              >
                <Camera size={18} /> Capture
              </button>
              <button
                onClick={closeCamera}
                className="w-full sm:w-auto px-6 py-3 border border-gray-200 hover:bg-gray-50 rounded-xl sm:rounded-2xl font-medium transition-colors"
              >
                Cancel
              </button>
            </div>
        ...
        <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 p-3 sm:p-5 w-full flex flex-col md:flex-row gap-6 md:gap-8 items-stretch">
          ...
          >
            ...
            <div className="z-10 flex flex-col items-center  justify-center">
              ...
              <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2 text-center tracking-tight">
                Scan Your Plant
              </h3>
          ...
          {/* Action Buttons & Info */}
          <div className="flex mt-15 flex-col md:w-[280px] gap-4 md:pr-4">
            <button
              onClick={() => fileInputRef.current?.click()}
            ...
            <div className="mt-4 flex items-start gap-3 p-2">
              <ShieldCheck
                size={24}
                className="text-[#166534] shrink-0 mt-0.5"
                strokeWidth={2}
              />
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl border border-gray-100 shadow-sm overflow-hidden max-w-2xl mx-auto">
          ...
          <div className="p-4 sm:p-6 flex flex-col sm:flex-row gap-4 sm:gap-6 items-center">
            <img
              src={preview}
              alt="preview"
              className="w-32 h-32 sm:w-48 sm:h-48 object-cover rounded-xl sm:rounded-2xl shadow-sm border border-gray-100"
            />
            <div className="flex flex-col w-full sm:w-auto items-center sm:items-start gap-3 sm:gap-4 text-center sm:text-left">
              <p className="text-gray-500 text-sm">
                Image ready for disease analysis
              </p>
              {!scanning ? (
                <button
                  onClick={handleScan}
                  className="w-full sm:w-auto bg-green-700 hover:bg-green-800 text-white px-8 py-3 rounded-xl sm:rounded-2xl font-bold shadow-md transition-all active:scale-95"
                >
                  Scan for Disease
                </button>
              ) : (
                <div className="flex flex-col items-center sm:items-start gap-2 w-full">
                  ...
                  <p className="text-sm text-gray-500 font-medium text-center sm:text-left w-full">
      ...
      {result && (
        <div className="bg-white rounded-2xl sm:rounded-3xl border border-gray-100 shadow-md overflow-hidden w-full">
          <div className="flex justify-between items-center px-4 sm:px-6 py-3 sm:py-4 border-b bg-gray-50/50">
            ...
            <button
              onClick={resetScan}
              className="text-green-700 text-xs sm:text-sm font-semibold hover:text-green-800 hover:underline"
            >
              Scan New Plant
            </button>
          </div>
            ...
            <div className="w-full lg:w-5/12 p-3 sm:p-5 border-b lg:border-b-0 lg:border-r border-gray-100 bg-gray-50/30 flex items-center justify-center">
              {preview && (
                <img
                  src={preview}
                  alt="Scanned Plant"
                  className="w-full h-56 sm:h-72 lg:h-[340px] object-cover rounded-xl sm:rounded-2xl shadow-sm border border-gray-200/60"
                />
              )}
            </div>
            ...
            <div className="w-full lg:w-7/12 p-4 sm:p-6 space-y-4 sm:space-y-5">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-xl sm:text-2xl font-bold leading-tight break-words ${isHealthy ? "text-green-700" : "text-red-600"}`}
                  >
                    {isHealthy ? "Healthy" : diseaseName || "Disease Detected"}
                  </p>
                  <span
                    className={`inline-block mt-1.5 px-2.5 sm:px-3 py-0.5 text-[10px] sm:text-xs font-semibold rounded-full border
                ...
                {isHealthy ? (
                  <CheckCircle
                    size={24}
                    className="text-emerald-500 flex-shrink-0 sm:w-[28px] sm:h-[28px]"
                  />
                ) : (
                  <AlertTriangle
                    size={24}
                    className="text-red-500 flex-shrink-0 sm:w-[28px] sm:h-[28px]"
                  />
                )}
              </div>
              ...
              {!isHealthy && (pesticide || recommendation) && (
                <>
                  <div className="pt-3 sm:pt-4 border-t border-gray-100">
                    <p className="text-[10px] sm:text-xs text-gray-400 mb-1 uppercase tracking-wider font-semibold">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 text-sm">
                      {dosage && (
                        <div className="flex items-center justify-between sm:block bg-blue-50/50 p-2 sm:p-3 rounded-lg border border-blue-100">
                          <p className="text-[10px] sm:text-xs text-blue-500/80 mb-0 sm:mb-0.5">
                            Dosage
                          </p>
                          <p className="font-medium text-blue-900 text-xs sm:text-sm text-right sm:text-left">
                            {dosage}
                          </p>
                        </div>
                      )}
                      {sprayInterval && (
                        <div className="flex items-center justify-between sm:block bg-purple-50/50 p-2 sm:p-3 rounded-lg border border-purple-100">
                          <p className="text-[10px] sm:text-xs text-purple-500/80 mb-0 sm:mb-0.5">
                            Interval
                          </p>
                          <p className="font-medium text-purple-900 text-xs sm:text-sm text-right sm:text-left">
                            {sprayInterval}
                          </p>
                        </div>
                      )}
                  ...
                  {recommendation && recommendation !== pesticide && (
                    <div className="bg-amber-50 border border-amber-100 rounded-xl sm:rounded-2xl p-3 sm:p-4">
                      <p className="text-[10px] sm:text-xs text-amber-700 font-bold mb-1 uppercase tracking-wider">
                        Expert Note
                      </p>
              ...
              {isHealthy && (
                <div className="bg-green-50 border border-green-100 rounded-xl sm:rounded-2xl p-3 sm:p-4">
                  <p className="text-sm sm:text-base text-green-800 font-semibold flex items-center gap-2">
                    <CheckCircle size={18} className="text-green-600" /> Plant
                    is Thriving!
                  </p>
                  <p className="text-xs sm:text-sm text-green-700 mt-1 sm:mt-1.5 opacity-90">
                    Your plant appears healthy. Keep monitoring regularly to
                    maintain crop yield.
                  </p>
                </div>
              )}
